...
export async function getQuestions(params: GetQuestionsParams) {
  try {
    ...
    // if the total number of questions is greater than the number of questions,then the next page exists.
    // if the total number of questions is equal to the number of questions, then the next page does not exist.
...
export async function upvoteQuestion(params: QuestionVoteParams) {
  try {
    ...
    // increment author's reputation by +1 or -1 for upvoting/downvoting
    await User.findByIdAndUpdate(userId, {
      $inc: { reputation: hasupVoted ? -1 : 1 },
    });

    // increment authors reputation by +10 or -10 for receiving and upvote/downvote
